...
import { Button, Card, CardHeader, CardTitle, CardDescription, Progress, Slider, Switch, Spinner, useToast } from '@/components/ui';
...
export default function ScoringPage() {
  ...
  // Load jobId from route params
  const { jobId } = useParams<{ jobId: string }>();
  // Load live scoring data
  const { data, isLoading, error } = useScoring(jobId);
  ...
  if (error) {
    ...
    return <div className="text-red-500">Failed to load scoring data.</div>;
  }
